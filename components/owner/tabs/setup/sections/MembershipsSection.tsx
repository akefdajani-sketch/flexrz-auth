...
export default function MembershipsSection({ ctx }: { ctx: any }) {
  ...
  const customerMemberships: CustomerMembership[] = Array.isArray(ctx?.customerMemberships) ? ctx.customerMemberships : [];
  const customerMembershipsLoading = Boolean(ctx?.customerMembershipsLoading);
  const customerMembershipsSaving = Boolean(ctx?.customerMembershipsSaving);
  const customerMembershipsMessage: string = ctx?.customerMembershipsMessage ?? "";
  const customerMembershipsError: string = ctx?.customerMembershipsError ?? "";
  const customerMembershipsMeta = ctx?.customerMembershipsMeta && typeof ctx.customerMembershipsMeta === "object" ? ctx.customerMembershipsMeta : { total: 0, limit: 25, offset: 0, hasMore: false };
  const customerMembershipsFilters = ctx?.customerMembershipsFilters && typeof ctx.customerMembershipsFilters === "object" ? ctx.customerMembershipsFilters : { q: "", status: "all", includeExpired: true, includeArchived: false, limit: 25, offset: 0 };
  const loadCustomerMemberships = typeof ctx?.loadCustomerMemberships === "function" ? ctx.loadCustomerMemberships : async () => {};
  const updateCustomerMembershipStatus = typeof ctx?.updateCustomerMembershipStatus === "function" ? ctx.updateCustomerMembershipStatus : async () => {};
  ...
  const [membershipSearch, setMembershipSearch] = React.useState<string>(String(customerMembershipsFilters?.q || ""));
  const [membershipStatusFilter, setMembershipStatusFilter] = React.useState<string>(String(customerMembershipsFilters?.status || "all"));
  const [membershipIncludeArchived, setMembershipIncludeArchived] = React.useState<boolean>(Boolean(customerMembershipsFilters?.includeArchived));
  const [membershipIncludeExpired, setMembershipIncludeExpired] = React.useState<boolean>(customerMembershipsFilters?.includeExpired !== false);
  ...
  React.useEffect(() => {
    setMembershipSearch(String(customerMembershipsFilters?.q || ""));
    setMembershipStatusFilter(String(customerMembershipsFilters?.status || "all"));
    setMembershipIncludeArchived(Boolean(customerMembershipsFilters?.includeArchived));
    setMembershipIncludeExpired(customerMembershipsFilters?.includeExpired !== false);
  }, [customerMembershipsFilters]);
  ...
  const customerMembershipCounts = {
    active: customerMemberships.filter((row) => row.lifecycle_state === "active").length,
    scheduled: customerMemberships.filter((row) => row.lifecycle_state === "scheduled").length,
    expired: customerMemberships.filter((row) => row.lifecycle_state === "expired").length,
    archived: customerMemberships.filter((row) => row.lifecycle_state === "archived" || row.status === "archived").length,
  };

  const applyCustomerMembershipFilters = async (nextOffset?: number) => {
    await loadCustomerMemberships(undefined as any, {
      q: membershipSearch,
      status: membershipStatusFilter,
      includeArchived: membershipIncludeArchived,
      includeExpired: membershipIncludeExpired,
      limit: Number(customerMembershipsMeta?.limit || customerMembershipsFilters?.limit || 25),
      offset: typeof nextOffset === "number" ? nextOffset : 0,
    });
  };

  const resetCustomerMembershipFilters = async () => {
    setMembershipSearch("");
    setMembershipStatusFilter("all");
    setMembershipIncludeArchived(false);
    setMembershipIncludeExpired(true);
    await loadCustomerMemberships(undefined as any, { q: "", status: "all", includeArchived: false, includeExpired: true, limit: Number(customerMembershipsMeta?.limit || 25), offset: 0 });
  };

  const lifecycleBadge = (value?: string | null) => {
    const key = String(value || "unknown").toLowerCase();
    const tone = key === "active" ? { bg: "#dcfce7", fg: "#166534" } : key === "scheduled" ? { bg: "#dbeafe", fg: "#1d4ed8" } : key === "expired" ? { bg: "#fef3c7", fg: "#92400e" } : key === "archived" ? { bg: "#e2e8f0", fg: "#475569" } : { bg: "#f1f5f9", fg: "#475569" };
    const label = key ? key.charAt(0).toUpperCase() + key.slice(1) : "Unknown";
    return <span style={{ padding: "3px 8px", borderRadius: 999, background: tone.bg, color: tone.fg, fontSize: 11, fontWeight: 900 }}>{label}</span>;
  };
      ...
      <div style={{ padding: 14, borderRadius: 14, border: "1px solid #e2e8f0", background: "#ffffff" }}>
        ...
        {prepaidTab === "plans" ? (
          <div style={{ display: "grid", gap: 12 }}>
            ...
            <div style={{ display: "grid", gap: 12 }}>
              <div>
                <div style={{ fontSize: 13, fontWeight: 900, color: "#0f172a" }}>Customer memberships</div>
                <div style={{ fontSize: 12, color: "#64748b", marginTop: 4 }}>Review which customers currently hold memberships, how much value is left, and whether a membership is active, scheduled, expired, or archived.</div>
              </div>

              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 10 }}>
                <div style={{ padding: 12, borderRadius: 14, border: "1px solid #e2e8f0", background: "#f8fafc" }}><div style={{ fontSize: 11, color: "#64748b", fontWeight: 800 }}>Active</div><div style={{ fontSize: 24, lineHeight: 1.1, fontWeight: 950, color: "#0f172a", marginTop: 6 }}>{customerMembershipCounts.active}</div></div>
                <div style={{ padding: 12, borderRadius: 14, border: "1px solid #e2e8f0", background: "#f8fafc" }}><div style={{ fontSize: 11, color: "#64748b", fontWeight: 800 }}>Scheduled</div><div style={{ fontSize: 24, lineHeight: 1.1, fontWeight: 950, color: "#0f172a", marginTop: 6 }}>{customerMembershipCounts.scheduled}</div></div>
                <div style={{ padding: 12, borderRadius: 14, border: "1px solid #e2e8f0", background: "#f8fafc" }}><div style={{ fontSize: 11, color: "#64748b", fontWeight: 800 }}>Expired</div><div style={{ fontSize: 24, lineHeight: 1.1, fontWeight: 950, color: "#0f172a", marginTop: 6 }}>{customerMembershipCounts.expired}</div></div>
                <div style={{ padding: 12, borderRadius: 14, border: "1px solid #e2e8f0", background: "#f8fafc" }}><div style={{ fontSize: 11, color: "#64748b", fontWeight: 800 }}>Archived</div><div style={{ fontSize: 24, lineHeight: 1.1, fontWeight: 950, color: "#0f172a", marginTop: 6 }}>{customerMembershipCounts.archived}</div></div>
              </div>

              <div style={{ padding: 12, borderRadius: 14, border: "1px solid #e2e8f0", background: "#ffffff", display: "grid", gap: 10 }}>
                <div style={{ display: "grid", gridTemplateColumns: "minmax(240px, 1.35fr) repeat(2, minmax(160px, 0.6fr)) auto auto", gap: 10, alignItems: "end" }}>
                  <div style={{ display: "grid", gap: 6 }}><div style={{ fontSize: 12, fontWeight: 800, color: "#0f172a" }}>Customer or plan</div><input type="text" value={membershipSearch} onChange={(e) => setMembershipSearch(e.target.value)} className="bf-input" style={{ height: 40 }} placeholder="Search name, phone, email, or plan" /></div>
                  <div style={{ display: "grid", gap: 6 }}><div style={{ fontSize: 12, fontWeight: 800, color: "#0f172a" }}>Lifecycle</div><select value={membershipStatusFilter} onChange={(e) => setMembershipStatusFilter(e.target.value)} className="bf-input" style={{ height: 40 }}><option value="all">All</option><option value="active">Active</option><option value="scheduled">Scheduled</option><option value="expired">Expired</option><option value="archived">Archived</option></select></div>
                  <div style={{ display: "grid", gap: 6 }}><div style={{ fontSize: 12, fontWeight: 800, color: "#0f172a" }}>Archived</div><label style={{ display: "flex", alignItems: "center", gap: 8, height: 40, fontSize: 12, color: "#0f172a", fontWeight: 800 }}><input type="checkbox" checked={membershipIncludeArchived} onChange={(e) => setMembershipIncludeArchived(e.target.checked)} /> Include archived</label></div>
                  <button type="button" onClick={() => void applyCustomerMembershipFilters()} style={{ border: "1px solid #0f172a", background: "#0f172a", color: "#fff", borderRadius: 999, padding: "10px 12px", fontSize: 12, fontWeight: 900 }}>Apply</button>
                  <button type="button" onClick={() => void resetCustomerMembershipFilters()} style={{ border: "1px solid #cbd5e1", background: "#fff", color: "#0f172a", borderRadius: 999, padding: "10px 12px", fontSize: 12, fontWeight: 900 }}>Reset</button>
                </div>

                {customerMembershipsError ? <div style={{ padding: "10px 12px", borderRadius: 14, border: "1px solid #fecaca", background: "#fef2f2", color: "#b91c1c", fontSize: 12, fontWeight: 800 }}>{customerMembershipsError}</div> : null}
                {customerMembershipsMessage ? <div style={{ padding: "10px 12px", borderRadius: 14, border: "1px solid #bbf7d0", background: "#f0fdf4", color: "#166534", fontSize: 12, fontWeight: 800 }}>{customerMembershipsMessage}</div> : null}

                {customerMembershipsLoading ? <div style={{ padding: "10px 12px", borderRadius: 14, border: "1px solid #e2e8f0", background: "#f8fafc", fontSize: 12, color: "#64748b", fontWeight: 700 }}>Loading customer memberships…</div> : null}

                {!customerMembershipsLoading && !customerMemberships.length ? <div style={{ padding: "12px 14px", borderRadius: 14, border: "1px solid #e2e8f0", background: "#f8fafc", fontSize: 12, color: "#64748b", lineHeight: 1.5 }}>No customer memberships found for the current filters.</div> : null}

                <div style={{ display: "grid", gap: 10 }}>
                  {customerMemberships.map((row) => (
                    <div key={row.id} style={{ padding: 12, borderRadius: 14, border: "1px solid #e2e8f0", background: "#f8fafc", display: "grid", gap: 8 }}>
                      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
                        <div style={{ display: "grid", gap: 4 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                            <div style={{ fontSize: 14, fontWeight: 950, color: "#0f172a" }}>{row.customer_name || "Customer"}</div>
                            {lifecycleBadge(row.lifecycle_state || row.status)}
                          </div>
                          <div style={{ fontSize: 12, color: "#64748b" }}>{row.customer_phone || row.customer_email || "No contact details"}</div>
                        </div>
                        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "flex-end" }}>
                          {(row.lifecycle_state === "archived" || row.status === "archived") ? (
                            <button type="button" disabled={customerMembershipsSaving} onClick={() => void updateCustomerMembershipStatus(Number(row.id), "active")} style={{ border: "1px solid #0f172a", background: "#0f172a", color: "#fff", borderRadius: 999, padding: "7px 12px", fontSize: 12, fontWeight: 900 }}>Restore</button>
                          ) : (
                            <button type="button" disabled={customerMembershipsSaving} onClick={() => void updateCustomerMembershipStatus(Number(row.id), "archived")} style={{ border: "1px solid #cbd5e1", background: "#fff", color: "#0f172a", borderRadius: 999, padding: "7px 12px", fontSize: 12, fontWeight: 900 }}>Archive</button>
                          )}
                        </div>
                      </div>
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(5, minmax(0, 1fr))", gap: 10 }}>
                        <div><div style={{ fontSize: 11, color: "#64748b", fontWeight: 800 }}>Plan</div><div style={{ fontSize: 13, fontWeight: 900, color: "#0f172a", marginTop: 3 }}>{row.plan_name || "Membership plan"}</div></div>
                        <div><div style={{ fontSize: 11, color: "#64748b", fontWeight: 800 }}>Minutes left</div><div style={{ fontSize: 13, fontWeight: 900, color: "#0f172a", marginTop: 3 }}>{row.minutes_remaining ?? 0}</div></div>
                        <div><div style={{ fontSize: 11, color: "#64748b", fontWeight: 800 }}>Uses left</div><div style={{ fontSize: 13, fontWeight: 900, color: "#0f172a", marginTop: 3 }}>{row.uses_remaining ?? 0}</div></div>
                        <div><div style={{ fontSize: 11, color: "#64748b", fontWeight: 800 }}>Starts</div><div style={{ fontSize: 13, fontWeight: 900, color: "#0f172a", marginTop: 3 }}>{formatDate(row.start_at || undefined)}</div></div>
                        <div><div style={{ fontSize: 11, color: "#64748b", fontWeight: 800 }}>Ends</div><div style={{ fontSize: 13, fontWeight: 900, color: "#0f172a", marginTop: 3 }}>{formatDate(row.end_at || undefined)}</div></div>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                  <div style={{ fontSize: 12, color: "#64748b" }}>Showing {customerMemberships.length} of {Number(customerMembershipsMeta?.total || 0)} memberships</div>
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    <button type="button" disabled={customerMembershipsLoading || Number(customerMembershipsMeta?.offset || 0) <= 0} onClick={() => void applyCustomerMembershipFilters(Math.max(0, Number(customerMembershipsMeta?.offset || 0) - Number(customerMembershipsMeta?.limit || 25)))} style={{ border: "1px solid #cbd5e1", background: "#fff", color: "#0f172a", borderRadius: 999, padding: "8px 12px", fontSize: 12, fontWeight: 900 }}>Previous</button>
                    <button type="button" disabled={customerMembershipsLoading || !customerMembershipsMeta?.hasMore} onClick={() => void applyCustomerMembershipFilters(Number(customerMembershipsMeta?.offset || 0) + Number(customerMembershipsMeta?.limit || 25))} style={{ border: "1px solid #0f172a", background: "#0f172a", color: "#fff", borderRadius: 999, padding: "8px 12px", fontSize: 12, fontWeight: 900 }}>Next</button>
                  </div>
                </div>
              </div>
            </div>
