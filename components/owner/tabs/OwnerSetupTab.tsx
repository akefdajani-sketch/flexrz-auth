...
export default function OwnerSetupTab(props: OwnerSetupTabProps) {
  ...
  const [customerMemberships, setCustomerMemberships] = React.useState<CustomerMembershipRecord[]>([]);
  const [customerMembershipsLoading, setCustomerMembershipsLoading] = React.useState<boolean>(false);
  const [customerMembershipsSaving, setCustomerMembershipsSaving] = React.useState<boolean>(false);
  const [customerMembershipsMessage, setCustomerMembershipsMessage] = React.useState<string | null>(null);
  const [customerMembershipsError, setCustomerMembershipsError] = React.useState<string | null>(null);
  const [customerMembershipsMeta, setCustomerMembershipsMeta] = React.useState<any>({ total: 0, limit: 25, offset: 0, hasMore: false });
  const [customerMembershipsFilters, setCustomerMembershipsFilters] = React.useState<any>({ q: "", status: "all", includeExpired: true, includeArchived: false, limit: 25, offset: 0 });
  const customerMembershipsLoadedForSlug = React.useRef<string | null>(null);
...
function customerMembershipsUrl(apiBase: string, tenantSlug: any, filters?: Record<string, any>): string {
  const params = new URLSearchParams();
  params.set("tenantSlug", String(tenantSlug || ""));
  if (filters) {
    if (filters.customerId) params.set("customerId", String(filters.customerId));
    if (filters.q) params.set("q", String(filters.q));
    if (filters.status && String(filters.status) !== "all") params.set("status", String(filters.status));
    if (typeof filters.includeExpired === "boolean") params.set("includeExpired", String(filters.includeExpired));
  ...
  return `${String(apiBase || "").trim()}/customer-memberships?${params.toString()}`;
}

function customerMembershipStatusUrl(apiBase: string, tenantSlug: any, membershipId: string | number): string {
  return `${String(apiBase || "").trim()}/customer-memberships/${encodeURIComponent(String(membershipId))}/status?tenantSlug=${encodeURIComponent(String(tenantSlug || ""))}`;
}
...
function normalizeMembershipPlans(input: any): MembershipPlanRecord[] {
  ...
  return plans
    ...
    .filter((plan: MembershipPlanRecord) => plan.id > 0 && Boolean(plan.name));
}

function normalizeCustomerMemberships(input: any): CustomerMembershipRecord[] {
  const memberships = Array.isArray(input?.memberships) ? input.memberships : Array.isArray(input) ? input : [];
  return memberships
    .filter((item: any) => item && typeof item === "object")
    .map((item: any) => ({
      id: Number(item.id || 0),
      ...
      customer_name: item.customer_name == null ? null : String(item.customer_name),
      customer_phone: item.customer_phone == null ? null : String(item.customer_phone),
      customer_email: item.customer_email == null ? null : String(item.customer_email),
      plan_id: item.plan_id == null ? null : Number(item.plan_id),
      plan_name: item.plan_name == null ? null : String(item.plan_name),
      plan_description: item.plan_description == null ? null : String(item.plan_description),
      plan_price: item.plan_price == null ? null : Number(item.plan_price),
      plan_currency: item.plan_currency == null ? null : String(item.plan_currency),
      included_minutes: item.included_minutes == null ? null : Number(item.included_minutes),
      included_uses: item.included_uses == null ? null : Number(item.included_uses),
      validity_days: item.validity_days == null ? null : Number(item.validity_days),
      status: item.status == null ? null : String(item.status),
      lifecycle_state: item.lifecycle_state == null ? null : String(item.lifecycle_state),
      is_currently_active: item.is_currently_active === true,
      start_at: item.start_at == null ? null : String(item.start_at),
      end_at: item.end_at == null ? null : String(item.end_at),
      minutes_remaining: item.minutes_remaining == null ? null : Number(item.minutes_remaining),
      uses_remaining: item.uses_remaining == null ? null : Number(item.uses_remaining),
      created_at: item.created_at == null ? null : String(item.created_at),
      updated_at: item.updated_at == null ? null : String(item.updated_at),
    }))
    .filter((row) => row.id > 0 && row.customer_id > 0);
}
...
const saveHomeLanding = async () => {
  ...
  const loadCustomerMemberships = async (tenantSlugInput?: string, nextFilters?: Record<string, any>) => {
    const tenantSlug = String(tenantSlugInput || localTenant?.slug || "");
    if (!tenantSlug) return;
    const merged = { ...customerMembershipsFilters, ...(nextFilters || {}) };
    setCustomerMembershipsLoading(true);
    setCustomerMembershipsError(null);
    setCustomerMembershipsMessage(null);
    setCustomerMembershipsFilters(merged);

    try {
      const res = await fetch(customerMembershipsUrl(apiBase, tenantSlug, merged), { method: "GET", cache: "no-store" });
      const json = await res.json().catch(() => ({} as any));
      if (!res.ok) throw new Error((json as any)?.error || "Failed to load customer memberships.");
      setCustomerMemberships(normalizeCustomerMemberships(json));
      setCustomerMembershipsMeta((json as any)?.meta || { total: 0, limit: merged.limit || 25, offset: merged.offset || 0, hasMore: false });
      customerMembershipsLoadedForSlug.current = tenantSlug;
    } catch (e: any) {
      setCustomerMembershipsError(e?.message || "Failed to load customer memberships.");
    } finally {
      setCustomerMembershipsLoading(false);
    }
  };

  const updateCustomerMembershipStatus = async (membershipId: number, status: "active" | "archived") => {
    const tenantSlug = String(localTenant?.slug || "");
    if (!tenantSlug || !membershipId) return;
    setCustomerMembershipsSaving(true);
    setCustomerMembershipsError(null);
    setCustomerMembershipsMessage(null);
    try {
      const res = await fetch(customerMembershipStatusUrl(apiBase, tenantSlug, membershipId), {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ status }),
      });
      const json = await res.json().catch(() => ({} as any));
      if (!res.ok) throw new Error((json as any)?.error || "Failed to update customer membership.");
      await loadCustomerMemberships(tenantSlug, customerMembershipsFilters);
      setCustomerMembershipsMessage(status === "archived" ? "Customer membership archived." : "Customer membership restored.");
    } catch (e: any) {
      setCustomerMembershipsError(e?.message || "Failed to update customer membership.");
    } finally {
      setCustomerMembershipsSaving(false);
    }
  };
  ...
  React.useEffect(() => {
    ...
    if (customerMembershipsLoadedForSlug.current !== tenantSlug) {
      loadCustomerMemberships(tenantSlug);
    }
...
const json = await res.json().catch(() => ({}));
  ...
  const membershipsCtx: any = {
    ...
    membershipPlansSaving,
    membershipPlansMessage,
    membershipPlansError,
    createMembershipPlan,
    updateMembershipPlan,
    archiveMembershipPlan,
    customerMemberships,
    customerMembershipsLoading,
    customerMembershipsSaving,
    customerMembershipsMessage,
    customerMembershipsError,
    customerMembershipsMeta,
