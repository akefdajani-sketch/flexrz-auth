...
export default function AuthSignInClient() {
  ...
  const appBase = (process.env.NEXT_PUBLIC_APP_BASE_URL || window.location.origin).replace(/\/$/, "");
  const callbackUrl = (() => {
    if (!rawCallbackUrl) return appBase + "/";
    if (rawCallbackUrl.startsWith("/")) return appBase + rawCallbackUrl;
    try {
      // If it's a full URL, keep it (auth will validate allowed hosts).
